 ...
 * @author mzechner
 * @author Nathan Sweet */
public class LinkedScrollPane extends WidgetGroup {
	private static final String TAG = "linked scroll pane";
	...
	private ScrollPaneGroup scrollPaneGroup;

	/** @param widget May be null. */
	public LinkedScrollPane(Actor widget) {
		this(widget, new ScrollPane.ScrollPaneStyle());
	}

	/** @param widget May be null. */
	public LinkedScrollPane(Actor widget, Skin skin) {
		this(widget, skin.get(ScrollPane.ScrollPaneStyle.class));
	}

	/** @param widget May be null. */
	public LinkedScrollPane(Actor widget, Skin skin, String styleName) {
		this(widget, skin.get(styleName, ScrollPane.ScrollPaneStyle.class));
	}

	/** @param widget May be null. */
	public LinkedScrollPane(Actor widget, ScrollPane.ScrollPaneStyle style) {
		...
		addCaptureListener(new InputListener() {
			...
			public boolean touchDown (InputEvent event, float x, float y, int pointer, int button) {
				...
				getStage().setScrollFocus(LinkedScrollPane.this);
				...
				if (scrollBarTouch && scrollX && hScrollBounds.contains(x, y)) {
					...
					setScrollX(amountX + areaWidth * (x < hKnobBounds.x ? -1 : 1), true);
					return true;
				}
				if (scrollBarTouch && scrollY && vScrollBounds.contains(x, y)) {
					...
					setScrollY(amountY + areaHeight * (y < vKnobBounds.y ? 1 : -1), true);
					return true;
				}
				return false;
			}
			...
			public void touchDragged (InputEvent event, float x, float y, int pointer) {
				...
				if (touchScrollH) {
					...
					if (total != 0) setScrollPercentX((scrollH - hScrollBounds.x) / total, true);
					lastPoint.set(x, y);
					...
					if (total != 0) setScrollPercentY(1 - ((scrollV - vScrollBounds.y) / total), true);
					lastPoint.set(x, y);
				}
			}
		...
		addListener(new InputListener() {
			public boolean scrolled (InputEvent event, float x, float y, int amount) {
				setScrollbarsVisible(true);
				if (scrollY)
					setScrollY(amountY + getMouseWheelY() * amount, true);
				else if (scrollX) //
					setScrollX(amountX + getMouseWheelX() * amount, true);
				else
					return false;
				return true;
			}
    ...
    private void setScrollX(float x, boolean propagate) {
        if (propagate && scrollPaneGroup != null) scrollPaneGroup.setScrollX(x, this);
        setScrollX(x);
		Gdx.app.log(TAG, "scrollpx");
    }

    private void setScrollY(float y, boolean propagate) {
        if (propagate && scrollPaneGroup != null) scrollPaneGroup.setScrollY(y, this);
        setScrollX(y);
		Gdx.app.log(TAG, "scrollpx");
    }
	...
	void clamp (boolean propagate) {
		if (!clamp) return;
		scrollX(overscrollX ? MathUtils.clamp(amountX, -overscrollDistance, maxX + overscrollDistance)
				: MathUtils.clamp(amountX, 0, maxX), propagate);
		scrollY(overscrollY ? MathUtils.clamp(amountY, -overscrollDistance, maxY + overscrollDistance)
				: MathUtils.clamp(amountY, 0, maxY), propagate);
	}
	...
	protected void scrollX (float pixelsX, boolean propagate) {
		if (propagate && scrollPaneGroup != null) scrollPaneGroup.scrollX(pixelsX, this);
		this.amountX = pixelsX;
	}

	/** Called whenever the y scroll amount is changed. */
	protected void scrollY (float pixelsY, boolean propagate) {
		if (propagate && scrollPaneGroup != null) scrollPaneGroup.scrollY(pixelsY, this);
	...
	public void setScrollPercentX (float percentX, boolean propagate) {
		if (propagate && scrollPaneGroup != null) scrollPaneGroup.setScrollPercentX(percentX, this);
	...
	public void setScrollPercentY (float percentY, boolean propagate) {
		if (propagate && scrollPaneGroup != null) scrollPaneGroup.setScrollPercentY(percentY, this);
	...
	public void setScrollPaneGroup(ScrollPaneGroup scrollPaneGroup) {
		this.scrollPaneGroup = scrollPaneGroup;
	}


	public static class ScrollPaneGroup{
		public Array<LinkedScrollPane> members = new Array<LinkedScrollPane>();
		public ScrollPaneGroup(){

		}

		public void add(LinkedScrollPane pane) {
			/*for (LinkedScrollPane m : members){
				m.
			}*/
			members.add(pane);
			pane.setScrollPaneGroup(this);
		}

		public void setScrollPercentX(float percentX, LinkedScrollPane source) {
			for (LinkedScrollPane pane : members){
				if (pane == source) continue;
				pane.setScrollPercentX(percentX);
			}
		}

		public void setScrollPercentY(float percentY, LinkedScrollPane source) {
			for (LinkedScrollPane pane : members){
				if (pane == source) continue;
				pane.setScrollPercentY(percentY);
			}
		}

        public void setScrollX(float x, LinkedScrollPane source) {
            for (LinkedScrollPane pane : members){
                if (pane == source) continue;
                pane.setScrollX(x);
            }
        ...
        public void setScrollY(float y, LinkedScrollPane source) {
            for (LinkedScrollPane pane : members){
                if (pane == source) continue;
                pane.setScrollY(y);
            }
		...
		public void scrollX(float pixelsX, LinkedScrollPane source) {
			for (LinkedScrollPane pane : members){
				if (pane == source) continue;
				pane.scrollX(pixelsX);
			}
		}

		public void scrollY(float pixelsY, LinkedScrollPane source) {
			for (LinkedScrollPane pane : members){
				if (pane == source) continue;
				pane.scrollY(pixelsY);
			}
		}
	}
}
